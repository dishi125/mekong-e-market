...
{{--            <td>{{$post->id}}</td>--}}
            ...
            @php
                ...
                isset($buyer_detail['bid_price'])?$buyer_detail['bid_price']=$buyer_detail['bid_price']:$buyer_detail['bid_price']=$post->starting_price;
                (isset($post->weight_unit_id)&&$post->weight_unit_id>0)?$post->unit=$post->unit:$post->unit='';
            @endphp
            <td class="tbltd">
                {!! isset($buyer_detail['buyer_name']) ? $buyer_detail['buyer_name']. '<br> <span class="text-gray-color">('.(($buyer_detail['transaction_status']) ? 'Payment Done' : 'Payment Pending'). ') </span>' : '-' !!}
            </td>
            <td class="tbltd">{{ $post->display_date_time }}</td>
            <td class="tbltd">{{ isset($buyer_detail['created_at']) ? $post->getDisplayEndDateTime($buyer_detail['created_at']) : $post->getDisplayEndDateTime($post->end_date) }}</td>
            <td class="tbltd">{{ 'RM'.$buyer_detail['bid_price'].'/'.$post->unit}}</td>
            <td class="tbltd">{{ (isset($post->qty)&&isset($post->weight_unit_id)) ? $post->weight : ''}}</td>
